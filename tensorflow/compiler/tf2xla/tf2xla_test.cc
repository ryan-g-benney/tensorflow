...
TEST(ConvertGraphDefToXla, Sum) {
  ...
  TF_ASSERT_OK(client_or.status());
  auto client = std::move(*client_or);
  auto compiler_or = xla::GetDefaultPjRtCompiler(xla::CpuName());
  TF_ASSERT_OK(compiler_or.status());
  auto* compiler = *compiler_or;
  ...
  ASSERT_OK_AND_ASSIGN(auto x_buf, client->BufferFromHostLiteral(
                                       x_literal, client->memory_spaces()[0]));
  ASSERT_OK_AND_ASSIGN(auto y_buf, client->BufferFromHostLiteral(
                                       y_literal, client->memory_spaces()[0]));
  ...
  ASSERT_OK_AND_ASSIGN(auto result, results[0][0]->ToLiteralSync());
...
TEST(ConvertGraphDefToXla, EinsumIsConvertedToDotWithDefaultPrecision) {
  ...
  xla::XlaComputation computation;
  TF_EXPECT_OK(
      ConvertGraphDefToXla(graph_def, config, *compiler_or, &computation));
