...
absl::Status AlgebraicSimplifierVisitor::HandleReduce(HloInstruction* hlo) {
  ...
  // Optimize reduce(add, multiply(A, broadcast(B)), init=0) to
  // multiply(reduce(add, A, init=0), B) when B does not depend on the
  // reduction dimensions (distributive law of multiplication over addition).
  // Only enabled when the multiplication has a single user.
  if ((ShapeUtil::ElementIsFloating(reduce->shape()) ||
       ShapeUtil::ElementIsIntegral(reduce->shape())) &&
      IsScalarConstantZero(init_value) &&
